...
                                                  |
                                              Throwable class : handles all errors and exceptions
                                                  |
                                                 / \
...
Note: 
1. There can be multiple catch blocks for one try block but the order matters: parent category cannot come before the child category --> "Catch(ArithmeticException e)" should come   before "Catch(Exception e)". Otherwise you get a compile-time exception something like "Exception is already caught".
2. The finally block is optional and there can only be one finally block.
3. You can also create your own exceptions by creating a class that extends Exception class.
